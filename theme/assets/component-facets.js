...
class FacetFiltersForm extends HTMLElement {
  ...
  static renderProductGrid(html) {
    ...
    // Re-initialize commonly used theme components for newly appended items.
    // Call specific init functions where available to keep re-init scoped and deterministic.
    try {
      var collectionGrid = document.getElementById('CollectionProductGrid');

      // Modal quick view / quick add to cart
      try { document.querySelector('modal-box quick-view-product')?.init?.(); } catch(e) {}
      try { document.querySelector('modal-box quick-add-to-cart')?.init?.(); } catch(e) {}
      try { document.querySelector('modal-box product-form')?.init?.(); } catch(e) {}

      // Product forms / variant selectors
      try { document.querySelectorAll('product-form').forEach(el=>el.init && el.init()); } catch(e) {}
      try { document.querySelectorAll('product-variants').forEach(el=>el.init && el.init()); } catch(e) {}

      // Quick buy, quantity selectors, image zoom, product item variants
      try { document.querySelectorAll('quick-add-to-cart').forEach(el=>el.init && el.init()); } catch(e) {}
      try { document.querySelectorAll('product-quantity').forEach(el=>el.init && el.init()); } catch(e) {}
      try { document.querySelectorAll('product-image-zoom').forEach(el=>el.init && el.init()); } catch(e) {}
      try { document.querySelectorAll('product-item-variants').forEach(el=>el.init && el.init()); } catch(e) {}

      // Reviews section re-init
      try { document.querySelectorAll('#shopify-product-reviews .spr-container').forEach(el=>{ if (window.ProductReviews) new ProductReviews(el.closest('#shopify-product-reviews')) }); } catch(e) {}

      // Fallback to theme-wide initializer if present
      if (window.theme && typeof window.theme.init === 'function') {
        try { window.theme.init(collectionGrid); } catch(e) {}
      }
    } catch (e) {
      // fail silently
    }
  ...
  static renderFilters(html, event) {
    ...
    const matchesIndex = (element) => {
      ...
      return jsFilter
        ? element.dataset.index === jsFilter.dataset.index && !element.hasAttribute('data-and-operator')
        : false;
    };
    ...
    if (this.parentElement.querySelector('[data-js-price-range-slider]')) {
      setTimeout(() => {
        ...
        rangeSlider(rangeSliderElm, {
          ...
          value: [
            inputMin.value || Number(inputMin.getAttribute('placeholder')),
            inputMax.value || Number(inputMax.getAttribute('placeholder')),
          ],
          onInput: (values) => {
            ...
            inputTimer = setTimeout(() => {
              document
                .querySelector(
                  `facet-filters-form[data-location="${rangeSliderElm.id.includes('desktop') ? 'desktop' : 'mobile'}"]`
                )
                .onSubmitHandler({
                  target: inputMin,
                  preventDefault: () => {
